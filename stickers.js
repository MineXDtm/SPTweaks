...
async function init() {
    ...
    skyb = await new THREE.GLTFLoader().loadAsync(browser.runtime.getURL("./skybox.gltf"));
...
(async () => {
    try {
        alex = await new THREE.GLTFLoader().loadAsync(browser.runtime.getURL("./alex.gltf"));
        steve = await new THREE.GLTFLoader().loadAsync(browser.runtime.getURL("./steve.gltf"));
    } catch (e) {
...
async function load_player_p(name, image, pose, backgraund = undefined, test = undefined) {
    ...
    if (playerbuffer[name] === undefined) {
        ...
        obj = object;
        texture = await new THREE.TextureLoader().setCrossOrigin("anonymous").loadAsync(pn.textures.skin.url);
...
function TEST(pose) {
    return fetch(browser.runtime.getURL("./poses/" + pose + ".json"))
        .then(response => response.json())
        .then(data => data);
}
...
function loadPOST(target) {
    ...
    b_icon.src = browser.runtime.getURL("emote_icon.png");
...
function sendcors(url) {
    return new Promise((resolve, reject) => {
        browser.runtime.sendMessage({ type: "CORS_HTTPREQUEST", url: url }, response => {
            if (response) {
...
function sticker_design_load(img, size) {
    img.setAttribute('style', '-webkit-user-drag: none')
    img.height = size;
    img.width = size;
    img.className = "sticker";
    img.src = browser.runtime.getURL("./load_sticker.svg")
    img.style.userSelect = "none"
    img.style.objectFit = "contain"

    img.style.backgroundColor = "rgba(45,45, 45, 1)";
    img.style.borderRadius = "15px"
    img.style.filter = "drop-shadow(0 2px gray) drop-shadow(0 -2px gray) drop-shadow(2px 0 gray) drop-shadow(-2px 0 gray)";
...
async function  enable(){
    ...
    link.href = browser.extension.getURL("./styles/sptweaks_default_style.css");
    link.type = "text/css";
    link.rel = "stylesheet";
    document.getElementsByTagName("head")[0].appendChild(link);
    let bg_i = document.createElement("div");
    //bg_i.style.backgroundImage = "url("+browser.runtime.getURL("./backgraund/19a27b959db7c85cff4506708e21be13.jpg")+")";
